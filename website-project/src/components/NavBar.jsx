    ...
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed top-0 z-50">
      ...
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          <li
            key={id}
            className="text-3xl px-4 cursor-pointer capitalize font-medium text-white hover:scale-105 duration-200"
          >
            <Link
              to={link}
              smooth
      ...
      {nav && (
        <ul
          className="flex flex-col justify-center items-center absolute top-0 w-full h-screen bg-gradient-to-b
          ...
          {links.map(({ id, link }) => (
            ...
            >
              <Link
                ...
                duration={500}
                offset={-70} // Adjust for fixed navbar height
                onClick={() => setNav(false)} // Close mobile menu on click
              >
                {link}
              </Link>
            </li>
